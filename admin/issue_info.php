...
function closeNav() {
 ...
 <?php
      ...
      {     
        ?>
        <h2 style="font-size: 40px; padding-top: 5px; height: 60px; width: 800px; margin-left: 100px; background-color: black; color: white;">Information Of Issued Books..</h2>
        <?php
    
             $sql1="SELECT * from `student`,`issue_book`,`books` where `books`.`bid`=`issue_book`.`bid` AND `student`.`username`=`issue_book`.`username` AND approve='yes' ";
    ...
    echo "<table class ='table table-bordered table-hover'>";
      ...
      { 
        ...
        { 
          $c=$c+1;
          mysqli_query($db,"UPDATE issue_book SET approve='Expired' where return_date='$row[return_date]' and approve='yes' limit $c; ");
        }
